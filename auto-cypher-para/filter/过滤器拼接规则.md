...
```
// {limit小于1表示不限制匹配子图个数}
RETURN olab.schema.auto.cypher({json},{skip},{limit}) AS autoCypher
```
...
RETURN olab.schema.auto.cypher({json},-1,-1) AS autoCypher
```
...
CREATE p6=(idu1)-[:上下游]->(idu2) WITH p1,p2,p3,p4,p5,p6
WITH olab.convert.json([p1,p2,p3,p4,p5,p6]) AS json
WITH olab.schema.auto.cypher(json,0,100) AS cypher
