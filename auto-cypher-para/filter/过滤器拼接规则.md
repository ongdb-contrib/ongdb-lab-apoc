...
### Elasticsearch过滤器
>主要实现对节点和关系的二级属性过滤查询【也可以支持一级属性】；被过滤对象和过滤条件之间使用`entity_unique_code:{var}`对接，`entity_unique_code`字段是默认的ONgDB和Elasticsearch数据对接的字段；依赖`custom.es.result.bool`函数执行ES过滤。
- `entity_unique_code`表示节点或者关系的实体唯一代码
```
"es_filter": [
          {
...
### MySQL过滤器【待添加ing】
>主要实现对节点和关系的二级属性过滤查询【也可以支持一级属性】；被过滤对象和过滤条件之间使用`entity_unique_code:{var}`对接，`entity_unique_code`字段是默认的ONgDB和MySQL数据对接的字段；依赖`存储过程返回true或者false`函数执行ES过滤。
```
"sql_filter": [
          {
            "jdbc_url": "jdbc_url",
