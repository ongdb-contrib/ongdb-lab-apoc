...
### 属性过滤器
>抽象定义对节点和关系的一级属性【暂不支持二级属性的过滤，可以通过存储过程的方式额外扩展】进行过滤；`{var}`表示对当前过滤对象的抽象定义；过滤条件之间支持使用AND连接。
```
"properties_filter": [
          {
...
### Elasticsearch过滤器
>主要实现对节点和关系的二级属性过滤查询【也可以支持一级属性】；被过滤对象和过滤条件之间使用`hcode:{var}`对接，`hcode`字段是默认的ONgDB和Elasticsearch数据对接的字段；依赖`custom.es.result.bool`函数执行ES过滤。
- `hcode`表示节点或者关系的实体唯一代码
```
// {var}表示cypher中的变量
...
"es_filter": [
          {
            ...
            "query": "{size:1,query:{term:{hcode:{var}.unique}}}}"
          }
]
```

### MySQL过滤器【待添加ing】
>主要实现对节点和关系的二级属性过滤查询【也可以支持一级属性】；被过滤对象和过滤条件之间使用`hcode:{var}`对接，`hcode`字段是默认的ONgDB和MySQL数据对接的字段；依赖`存储过程返回true或者false`函数执行ES过滤。
```
"sql_filter": [
          {
            "jdbc_url": "jdbc_url",
            "query": "SELECT resultBool(\"SQL\") AS bool"
          }
]
```
