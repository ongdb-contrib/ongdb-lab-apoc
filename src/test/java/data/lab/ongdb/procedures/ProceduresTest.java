...
//        Result result = db.execute("RETURN olab.samplingByDate.filter.jsonArray({jsonString},{dateField},{dateValue},{filterMap}) AS value", hashMap);
        Result result = db.execute("RETURN olab.samplingByDate.dis.filter.jsonArray({jsonString},{dateField},{dateValue},{filterMap}) AS value", hashMap);
        String string = (String) result.next().get("value");
        System.out.println(string);
    }
...
//        Result result = db.execute("RETURN olab.string.encode({string}) AS value", hashMap);
    ...
    @Test
    public void dateSamplingDateDisJsonArray() {
        GraphDatabaseService db = neo4j.getGraphDatabaseService();
        ...
        hashMap.put("jsonString", jsonString);
        // 时间值
        hashMap.put("dateValue", 20180630000000L);

        // 时间字段
        hashMap.put("dateField", "releaseDate");

        Result result = db.execute("RETURN olab.samplingByDate.dis.jsonArray({jsonString},{dateField},{dateValue}) AS value", hashMap);
        String string = (String) result.next().get("value");
        System.out.println(string);
    }
}
