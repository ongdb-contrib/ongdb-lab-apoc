...
public class AutoCypherWithIndicator {
    ...
    private final static String FUNC_VAR_MARK = "{var}.entity_unique_code";

    private final static String ENTITY_UNIQUE_CODE = "entity_unique_code";
    private final static String ENTITY_UNIQUE_VALUE = "entity_unique_value";
     ...
     *                                              "1112350872": [
     *                                              {
     *                                              "es_filter": "custom.es.result.bool('10.20.13.130:9200','gh_ind_rel_company_guarantee_company',{size:1,query:{bool:{filter:{bool:{must:[{range:{amount:{gte:100000}}}]}},must:[{term:{entity_unique_code:{var}.entity_unique_code}}]}}})"
     *                                              }
     *                                              ]
     *                                              }
     ...
     *                                              {var}.entity_unique_code 替换为生成值的查询
     *                                              ```
     * @return
    ...
    private Stream<VirtualPathResult> vpathInd(Relationship relationship, Long atomicId, Map<String, Object> vFMap) {
        ...
        revFMap.put(VFMAP_KEY_PREFIX + fId, excuteQueryInd(parseESQuery(vFMap.get(VFMAP_KEY_PREFIX + fId), fromNode.getAllProperties().get(ENTITY_UNIQUE_CODE))));
        revFMap.put(VFMAP_KEY_PREFIX + tId, excuteQueryInd(parseESQuery(vFMap.get(VFMAP_KEY_PREFIX + tId), toNode.getAllProperties().get(ENTITY_UNIQUE_CODE))));
        revFMap.put(VFMAP_KEY_PREFIX + rId, excuteQueryInd(parseESQuery(vFMap.get(VFMAP_KEY_PREFIX + rId), relationship.getAllProperties().get(ENTITY_UNIQUE_CODE))));
    ...
    private Map<String, Object> addMap(Map<String, Object> allProperties, Map<String, Object> stringObjectMap) {
        ...
        if (stringObjectMap != null && !stringObjectMap.isEmpty()) {
            ...
            allProperties.put(ENTITY_UNIQUE_VALUE, jsonArray.toJSONString());
        }
